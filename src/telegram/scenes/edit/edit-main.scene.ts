...
export default (editMain: Scenes.BaseScene<BotContext>): void => {
    editMain.enter(async (ctx: BotContext) => {
    ...
    editMain.action('createTraining', async (ctx: BotContext) => {
        return await ctx.scene.enter(SCENES.CREATE_TRAINING);
    });

    editMain.action(/editTraining-.*/, async (ctx: BotMatchContext) => {
    ...
    editMain.action('toMainMenu', async (ctx: BotContext) => {
